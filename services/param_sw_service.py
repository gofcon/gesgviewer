...
class ParamSwService:
    ...
    def get_sw_usr_list(appl_st_yymm: str = "", appl_biz_dv: str = "",
        ...
        with get_session() as session:
            ...
            total = q.count()
            rows = q.order_by(IrParamSwUsr.appl_st_yymm, IrParamSwUsr.appl_biz_dv)\
    ...
    def save_sw_usr(data: dict) -> None:
        with get_session() as session:
            ...
            row = session.get(IrParamSwUsr, pk)
            if row:
                for k, v in data.items():
                    if hasattr(row, k):
                        setattr(row, k, v)
            else:
                session.add(IrParamSwUsr(**data))
            session.commit()
    ...
    def delete_sw_usr(appl_st_yymm: str, appl_biz_dv: str,
        ...
        with get_session() as session:
            row = session.get(IrParamSwUsr, (appl_st_yymm, appl_biz_dv, ir_curve_id, ir_curve_sce_no))
            if row:
                session.delete(row)
                session.commit()
    ...
    def get_param_model_list(page: int = 1, page_size: int = 20) -> tuple[list[dict], int]:
        with get_session() as session:
            ...
            total = q.count()
            rows = q.order_by(IrParamModel.ir_model_id)\
                    .offset((page - 1) * page_size).limit(page_size).all()
            return ([{
    ...
    def save_param_model(data: dict) -> None:
        with get_session() as session:
            row = session.get(IrParamModel, (data["ir_model_id"], data["ir_curve_id"]))
            if row:
                for k, v in data.items():
                    if hasattr(row, k):
                        setattr(row, k, v)
            else:
                session.add(IrParamModel(**data))
            session.commit()

    @staticmethod
    def delete_param_model(ir_model_id: str, ir_curve_id: str) -> None:
        with get_session() as session:
            row = session.get(IrParamModel, (ir_model_id, ir_curve_id))
            if row:
                session.delete(row)
                session.commit()
    ...
    def get_ir_curve_list(page: int = 1, page_size: int = 20) -> tuple[list[dict], int]:
        with get_session() as session:
            ...
            total = q.count()
            rows = q.order_by(IrCurve.ir_curve_id)\
                    .offset((page - 1) * page_size).limit(page_size).all()
            return ([{
                "ir_curve_id": r.ir_curve_id,
