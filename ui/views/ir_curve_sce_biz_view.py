...
from db.models.ir_curve import IrCurveSceBiz
...
class IrCurveSceBizView(BaseListView):
    HEADERS = HEADERS
    VIEW_TITLE = "금리커브 시나리오(비즈)"
    ...
    def _load_data(self, page: int = 1):
        with get_session() as session:
            q = session.query(IrCurveSceBiz)
            ym = self._get_toolbar_yymm()
            if ym:
                q = q.filter(IrCurveSceBiz.base_yymm == ym)
            total = q.count()
