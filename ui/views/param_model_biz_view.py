...
from db.models.ir_param_model import IrParamModelBiz
...
class ParamModelBizView(BaseListView):
    HEADERS = HEADERS
    VIEW_TITLE = "파라미터 모델 비즈"
    ...
    def _load_data(self, page: int = 1):
        with get_session() as session:
            q = session.query(IrParamModelBiz)
            ym = self._get_toolbar_yymm()
            if ym:
                q = q.filter(IrParamModelBiz.base_yymm == ym)
            total = q.count()
