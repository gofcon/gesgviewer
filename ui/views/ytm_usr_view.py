...
class YtmUsrView(BaseCrudView):
    ...
    def __init__(self, user: AppUser, parent=None):
        ...
        self.ADD_FIELDS = [
            ...
            FormField("ytm",         "YTM금리",  "decimal"),
        ]
        super().__init__(user, parent)

    def _load_data(self, page: int = 1):
        rows, total = IrCurveInputService.get_ytm_usr_list(
    ...
    def _save_row(self, data: dict, is_new: bool) -> bool:
        IrCurveInputService.save_ytm_usr(data)
        return True

    def _delete_row(self, row: dict) -> bool:
        IrCurveInputService.delete_ytm_usr(row["base_date"], row["ir_curve_id"], row["mat_cd"])
        return True
