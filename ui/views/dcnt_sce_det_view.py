...
from db.models.dcnt_rate import IrDcntSceDet
...
class DcntSceDetView(BaseListView):
    HEADERS = HEADERS
    VIEW_TITLE = "확률시나리오 상세(비즈)"
    ...
    def _load_data(self, page: int = 1):
        with get_session() as session:
            q = session.query(IrDcntSceDet)
            ym = self._get_toolbar_yymm()
            if ym:
                q = q.filter(IrDcntSceDet.base_yymm == ym)
            total = q.count()
