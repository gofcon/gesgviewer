...
from db.models.ir_param_model import IrParamAfnsCalc
...
class ParamAfnsCalcView(BaseListView):
    HEADERS = HEADERS
    VIEW_TITLE = "AFNS 모수 계산"
    ...
    def _load_data(self, page: int = 1):
        with get_session() as session:
            q = session.query(IrParamAfnsCalc)
            ym = self._get_toolbar_yymm()
            if ym:
                q = q.filter(IrParamAfnsCalc.base_yymm == ym)
            total = q.count()
