...
from db.models.dcnt_rate import IrDcntSceIm
...
class DcntSceImView(BaseListView):
    HEADERS = HEADERS
    VIEW_TITLE = "확률시나리오 내부모형"
    ...
    def _load_data(self, page: int = 1):
        with get_session() as session:
            q = session.query(IrDcntSceIm)
            ym = self._get_toolbar_yymm()
            if ym:
                q = q.filter(IrDcntSceIm.base_yymm == ym)
            total = q.count()
