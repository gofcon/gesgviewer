"""AFNS 모수생성결과 화면 — JSP: ParamAfnsBizList.jsp"""
from ui.views._base_list_view import BaseListView
from db.database import get_session
from db.models.ir_param_model import IrParamAfnsBiz
...
class ParamAfnsView(BaseListView):
    HEADERS = HEADERS
    VIEW_TITLE = "AFNS 모수생성결과"
    def _load_data(self, page: int = 1):
        with get_session() as session:
            q = session.query(IrParamAfnsBiz)
            ym = self._get_toolbar_yymm()
            if ym: q = q.filter(IrParamAfnsBiz.base_yymm == ym)
            total = q.count()
