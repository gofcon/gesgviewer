...
from db.models.sprd_curve import SprdLp
from db.models.auth import AppUser
from config.settings import DEFAULT_PAGE_SIZE

HEADERS = [
    ("base_yymm","기준년월"), ("dcnt_appl_model_cd","할인모델"),
...
class SprdLpView(BaseListView):
    ...
    def _load_data(self, page: int = 1):
        with get_session() as session:
            q = session.query(SprdLp)
            ym = self._get_toolbar_yymm()
            if ym:
                q = q.filter(SprdLp.base_yymm == ym)
            total = q.count()
            ...
            rows = [
                {
                    ...
                    "dcnt_appl_model_cd": r.dcnt_appl_model_cd,
